...
# Set Paths (Vercel only allows writing to /tmp)
if IS_VERCEL:
    BASE_DIR = Path("/tmp")
else:
    BASE_DIR = Path(__file__).resolve().parent
...
# --- FFmpeg AUTO-INSTALLER (FOR VERCEL) ---
def install_ffmpeg_if_needed():
    """Downloads and configures FFmpeg for Vercel environment"""
    if not IS_VERCEL:
        return  # Skip on local machine

    ffmpeg_path = BASE_DIR / "ffmpeg"
    
    # If binary doesn't exist, download it
    if not ffmpeg_path.exists():
        print("📥 Vercel: Downloading FFmpeg...")
        try:
            ...
            # Extract
            print("📦 Vercel: Extracting FFmpeg...")
            subprocess.run(["tar", "-xJf", str(tar_path), "-C", str(BASE_DIR)], check=True)
            
            # Move binary to easy location
            # The tar contains a folder like 'ffmpeg-6.0-amd64-static', we need to find the bin inside
            ...
            print("✅ Vercel: FFmpeg installed successfully.")

        except Exception as e:
            print(f"❌ Vercel FFmpeg Error: {e}")

    # Set Permission and Environment
...
app = Flask(__name__)
app.config['SECRET_KEY'] = 'hanuman_secret'
# Async mode threading is required for Vercel/Serverless environments
socketio = SocketIO(app, cors_allowed_origins="*", async_mode='threading')
    ...
    try:
        with open(audio_path, "rb") as f:
            resp = requests.post(
                ...
                timeout=15 # Increased timeout for Vercel latency
            )
        if resp.status_code == 200:
            text = resp.json().get("text", "").strip()
            if text.lower() in ["you", "thank you", "bye", "am i", "mbc news"]: return ""
...
def transcribe_smart(audio_path):
    ...
    with ThreadPoolExecutor(max_workers=2) as executor:
        ...
        cmd, score = identify_command(turbo_text)
        if cmd:
            log_ui(f"✅ Fast match: {cmd.upper()}", "success")
            return turbo_text
...
def upload():
    ...
    # Save Temp File (using secure filename and time to avoid collision)
    f = request.files['audio']
    ...
    try:
        ...
        # Cleanup input audio immediately
        if path.exists(): path.unlink()
        ...
        if path.exists(): path.unlink() # Ensure cleanup on error
        log_ui(f"Critical Error: {e}", "error")
        return jsonify({"error": str(e)})
